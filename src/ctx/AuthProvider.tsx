...
import {httpAuthGetWithCreds} from '../core/http.client.ts'
...
export const AuthProvider = ({children}: { children: ReactNode }) => {
    ...
    const logout = async () => {
        await httpAuthGetWithCreds<{ message: string }>('/api/v1/logout')
    ...
    const refreshOnReload: AuthContextParams['refreshOnReload'] = async ({
        ...
        const {data} = await httpAuthGetWithCreds<AuthRes>('/api/v1/refresh')
        if (data) {
