                ...
                <Box width={itemWidth} pb={1}>
                    <Typography variant="body2">
                        <Link style={{textDecoration: 'none'}}
                              to={resetRoute}
                              state={{noRefresh: true}}>Forgot password?</Link>
                    </Typography>
                </Box>
                <ContinueButton width={itemWidth} formik={formik}/>
                <Typography variant="body2">Don't have an account? <Link style={{textDecoration: 'none'}}
                                                                         to={signUpRoute}
                                                                         state={{noRefresh: true}}>Sign up</Link>
                </Typography>
                {fetchResult?.errorDetails && <ErrorStatus label={fetchResult.errorDetails}/>}
                {fetchResult?.errorStatus === HttpStatusCode.Conflict &&
                    <Typography variant="body2">Forgot to <Link style={{textDecoration: 'none'}}
                                                                to={confirmSignupRoute}
                                                                state={{
                                                                    email: formik.values.email,
                                                                    noRefresh: true
                                                                }}>confirm</Link> your account?
                    </Typography>}
            </Stack>
        </form>
    )
